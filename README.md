...
### Why not mamba?
Since Mamba is in its infant state in R&D right now. It won't receive x2 speed boost from [unsloth](https://huggingface.co/docs/trl/en/sft_trainer#accelerate-fine-tuning-2x-using-unsloth), x6 speed boost from [FlashAttention-2](https://huggingface.co/docs/trl/en/sft_trainer#using-flash-attention-2). Also to be fair, I'm not sure if [QLora which worked for Transformer](https://github.com/huggingface/alignment-handbook/pull/88) will also work for Mamba, but that's why we are here - to test and share.
